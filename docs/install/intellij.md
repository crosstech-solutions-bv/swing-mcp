# Swing MCP in IntelliJ IDEA (JetBrains AI Assistant)

Video: <https://crosstech.solutions/swing-mcp#intellij> (about a minute)
...
2. Click **+** (Add), choose **STDIO**, and paste the configuration below. Choose **Global**
   or **Project** level.
3. **OK**, then **Apply**.
...
The same panel has an **Import from Claude** button that imports servers from
`claude_desktop_config.json`; if you installed Swing MCP in Claude Desktop as the `.mcpb`
extension rather than by editing that file, paste the JSON above instead.

## 3. Verify

The server appears in the list; click its status icon to see the available tools. Start a
new AI Assistant chat — the Swing tools are offered to the model.
