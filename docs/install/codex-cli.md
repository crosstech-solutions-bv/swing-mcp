# Swing MCP in OpenAI Codex CLI

Video: <https://crosstech.solutions/swing-mcp#codex> (about a minute)
...
Output: `Added global MCP server 'swing'.` This writes to `~/.codex/config.toml`:
...
# swing  java     -jar /path/to/…/swing-mcp-server.jar   SWING_MCP_AGENT_JAR=*****  enabled
codex mcp get swing
```
