# Swing MCP in VS Code (GitHub Copilot)

Video: <https://crosstech.solutions/swing-mcp#vscode> (about a minute)
...
**Workspace:** create `.vscode/mcp.json` (or run **MCP: Add Server** from the Command Palette
and pick *Workspace*):
...
Note the VS Code shape: top-level key `servers` (not `mcpServers`) and `"type": "stdio"`.

**All workspaces:** run **MCP: Open User Configuration** and add the same entry to your
user-profile `mcp.json`. **From a shell:**

```bash
code --add-mcp '{"name":"swing","command":"java","args":["-jar","/path/to/swing-mcp/server/swing-mcp-server.jar"],"env":{"SWING_MCP_AGENT_JAR":"/path/to/swing-mcp/server/swing-mcp-agent.jar"}}'
```
